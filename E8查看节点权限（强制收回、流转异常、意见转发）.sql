select b.id                 as 流程id,
       ...
       d.nodename           as 节点名称,
       c.retract            as 强制收回,
       IsPendingForward     as 是否允许待办事宜转发,
       ...
       ExceptionHandleWay   as 自动流转至下一节点--or由用户指定操作者or提交至指定节点  --字符串过长
  from workflow_flownode a
  left join workflow_base b
    on a.workflowid = b.id
  left join workflow_function_manage c
    on a.workflowid = c.workflowid
   and a.nodeid = c.operatortype
