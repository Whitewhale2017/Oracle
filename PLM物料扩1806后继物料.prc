create or replace procedure proc_expand_nf_1806(v_pbompkgid varchar2,v_operator nvarchar2)--扩展后继物料
as
...
begin
       ...
       ( select id from (
                ...
                from  (
                  ...
                  inner join (select NFMAT,nfwerks from SIPM170 where pbompkgid=v_pbompkgid) t1
                  on a1.werks=t1.nfwerks and (a1.no = t1.NFMAT)
                  and not exists(select 1 from  TEMP_WK_PARTLIB t where a1.no=t.no and a1.werks=t.werks)
                 ) t1
